"""
worker.py ÔÇö Worker individual para procesos multi-worker
=========================================================
...
# ÔöÇÔöÇ Supabase helpers ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
def _api(method: str, path: str, body: dict = None) -> list | dict:
    ...
    try:
        ...
        log(f"Supabase {method} {path} ÔåÆ {e.code}: {err}")
        return []
...
def actualizar_progreso_documento(dni: str):
    ...
    try:
        ...
        for doc in docs:
            doc_id = doc["id"]
            # Obtener l├¡nea para ver si pertenece a este documento
...
def reportar_actividad(dni_actual: str = ""):
    """Reporta el DNI que est├í procesando este worker en la tabla maquinas."""
    try:
...
def tomar_siguiente_dni() -> dict | None:
    """Toma el pr├│ximo DNI pendiente de la cola.
...
def guardar_resultado(dni: str, datos: dict, estado: str = "completado", linea_id: int = None):
    ...
    Si linea_id se pasa, actualiza esa fila espec├¡fica (evita bug de DNI duplicados).
    ...
    # ÔÜá´©Å Preservar renove_mixto_variante si ya existe uno mejor (no pisar con N/A)
    for k, v in ad.items():
        if k == "renove_mixto_variante" and k in ad_prev:
            # Solo actualizar si el nuevo valor NO es N/A (no pisar datos v├ílidos)
            if v not in (None, "N/A", ""):
                ad_prev[k] = v
        elif k == "tiene_renove_mixto" and k in ad_prev:
            # True se preserva (si alguna l├¡nea tiene Renove, el cliente lo tiene)
            if v:
                ad_prev[k] = True
        else:
            ad_prev[k] = v
...
# ÔöÇÔöÇ Procesar un DNI ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
def procesar_dni(page, dni: str, linea_id: int = None, modal_ya_abierto: bool = False) -> tuple:
    ...
    try:
        ...
        # Si todas las filas tienen N/A (no carg├│ la info), marcar como error
        todas_na = all(
        ...
        # Guardar cada fila (l├¡nea del cliente)
        for fila in filas:
        ...
        log(f"[OK]  {dni}: {len(filas)} l├¡neas")
        return True, False

    except Exception as e:
        log(f"[ERR]  {dni}: {e}")
        # ÔöÇÔöÇ Reintentar autom├íticamente (hasta 3 veces) ÔöÇÔöÇ
        import json as _json
        reintentos = 0
        try:
            existentes = _api("GET", f"/lineas?select=atributos_dinamicos,id&dni=eq.{dni}&limit=1&order=id.desc")
...
def main():
    ...
    procesados = 0
    errores = 0
    modal_abierto = False  # Para "no es cliente" ÔÇö mantener modal abierto
    ...
    try:
        ...
        for intento in range(5):
            try:
                ...
                log(f"[RETRY] Login fall├│ (intento {intento+1}/5): {e}")
                if intento == 4:
                    log("[CRIT] No se pudo iniciar sesi├│n tras 5 intentos. Esperando...")
                    # No crashear ÔÇö esperar y reintentar
                    time.sleep(30)
                    continue
                # Recuperar: recargar p├ígina
                try:
        ...
        # Loop de procesamiento ÔÇö NUNCA sale del while, solo espera si no hay DNIs
        while not detener:
            dni = "???"
            try:  # ­ƒöä Inner try: cualquier error aqu├¡ reloguea en vez de cerrar
                ...
                if not fila:
                    # ÔÜí NO romper el while ÔÇö solo esperar a que lleguen m├ís DNIs
                    # El browser sigue abierto, la sesi├│n de Pangea se mantiene
                    reportar_actividad("")
                    time.sleep(5)
                    continue
                ...
                if exito:
                    ...
                    errores += 1
                    modal_abierto = False
                    # Intentar abrir modal de b├║squeda
                    try:
                        ...
                        try:
                            log(f"[RECOVERY] No se pudo abrir modal. Recreando p├ígina...")
                            page.close()
                            page = context.new_page()
                            page.goto(ORANGE_URL, timeout=30000)
                            manejar_cookies_flexible(page)
                            ...
                            seleccionar_marca_orange(page)
                            abrir_nuevo_acto_comercial(page)
                            log("[RECOVERY] P├ígina recreada correctamente")
                            modal_abierto = False
                        except Exception as recovery_err:
                            log(f"[RECOVERY] Error al recrear p├ígina: {recovery_err}")

                    # Verificar sesi├│n cada 3 DNIs
                    if (procesados + errores) % 3 == 0:
                        if not verificar_sesion_valida(page):
                            log("[RETRY] Sesi├│n expirada, relogueando...")
                            page.goto(ORANGE_URL, timeout=30000)
                            realizar_login(page)
                            seleccionar_marca_orange(page)
                            abrir_nuevo_acto_comercial(page)
                    ...
                    if MAX_DNIS > 0 and (procesados + errores) >= MAX_DNIS:
                        log(f"­ƒÅü L├¡mite alcanzado ({MAX_DNIS} DNIs). Esperando...")
                        time.sleep(60)
                        procesados = 0
                        errores = 0

            except Exception as e:  # ­ƒöä CUALQUIER error: reloguear, no cerrar
                log(f"[ERR] Error grave en DNI {dni}: {e}")
                log("[RECOVERY] Relogueando autom├íticamente...")
                try:
                    page.close()
                except:
                    pass

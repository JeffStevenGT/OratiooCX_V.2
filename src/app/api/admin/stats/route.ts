...
export async function GET() {
  try {
    const [clientes, pendientes, completados, errores, maquinas, proxies, workers] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) FROM proxies"),
    ...
    return NextResponse.json({
      ...
      totalProxies: parseInt(proxies.rows[0].count),

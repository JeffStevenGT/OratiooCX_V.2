      ...
      <div className="flex gap-2 border-b border-[#e8dce6]">
        <button onClick={() => setTab('proxies')}
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
            tab === 'proxies' ? 'border-[#0a6ea9] text-[#0a6ea9]' : 'border-transparent text-[#7c757c] hover:text-[#1a1030]'
          }`}>Proxies ({proxies.length})</button>
        <button onClick={() => setTab('maquinas')}
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
            tab === 'maquinas' ? 'border-[#0a6ea9] text-[#0a6ea9]' : 'border-transparent text-[#7c757c] hover:text-[#1a1030]'
          }`}>Máquinas ({maquinas.length})</button>
      </div>
      ...
      {tab === 'proxies' && (
        <>
          ...
          <div className="card !p-0 overflow-hidden">
            ...
            {loadingProxies ? (
              <div className="flex items-center justify-center py-12"><Loader2 size={24} className="animate-spin text-[#b8b0b8]" /></div>
            ) : proxies.length === 0 ? (
              <div className="text-center py-12"><Wifi size={40} className="text-[#b8b0b8] mx-auto mb-2" /><p className="text-sm text-[#7c757c]">No hay proxies</p></div>
            ) : (
        ...
        <>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-4 gap-2 mb-3">
              <input placeholder="Nombre (ej: vps-espana-1)" value={newMachine.nombre}
              ...
              <input placeholder="IP" value={newMachine.ip}
                ...
                className="border border-[#e0e0f0] rounded-lg px-3 py-2 text-sm" />
              <input type="number" min={1} max={20} placeholder="Workers máx" value={newMachine.workers}
                onChange={e => setNewMachine({ ...newMachine, workers: +e.target.value })}
                className="border border-[#e0e0f0] rounded-lg px-3 py-2 text-sm" />
            </div>
            <input placeholder="Notas (opcional)" value={newMachine.notas}
              onChange={e => setNewMachine({ ...newMachine, notas: e.target.value })}
              className="border border-[#e0e0f0] rounded-lg px-3 py-2 text-sm w-full mb-3" />
          ...
          <div className="card !p-0 overflow-hidden">
            ...
            {loadingMaquinas ? (
              ...
              <div className="text-center py-12"><Monitor size={40} className="text-[#b8b0b8] mx-auto mb-2" /><p className="text-sm text-[#7c757c]">No hay máquinas registradas</p></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead><tr className="border-b border-[#e8dce6] bg-[#f8f7fa]">
                    <th className="table-header px-4 py-2.5 text-left">Nombre</th>
                    <th className="table-header px-4 py-2.5 text-left">IP</th>
                    <th className="table-header px-4 py-2.5 text-center">Estado</th>
                    <th className="table-header px-4 py-2.5 text-center">Workers</th>
                    <th className="table-header px-4 py-2.5 text-left">Último heartbeat</th>
                    <th className="table-header px-4 py-2.5 text-center w-16"></th>
                  </tr></thead>
                  <tbody>
                    {maquinas.map(m => (
                      <tr key={m.id} className="border-b border-[#f0f0f8] hover:bg-[#f8f7fa]">
                        ...
                        <td className="py-2.5 px-4 text-center">
                          <span className={`inline-flex items-center gap-1 text-[10px] font-medium rounded-full px-2 py-0.5 ${
                            m.estado === 'online' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                          }`}>
                        ...
                        <td className="py-2.5 px-4 text-center text-xs">
                          <span className="font-medium">{m.workers_activos}</span>
                          <span className="text-[#7c757c]">/{m.workers_max}</span>
                        </td>
                        <td className="py-2.5 px-4 text-xs text-[#7c757c]">
                          {m.ultimo_heartbeat ? new Date(m.ultimo_heartbeat).toLocaleString('es-PE') : '—'}
                        </td>
                        <td className="py-2.5 px-4 text-center">
                          <button onClick={() => handleDeleteMachine(m.id)} className="p-1.5 rounded hover:bg-red-50" title="Eliminar">
                            <Trash2 size={14} className="text-red-400 hover:text-red-600" />
                          </button>
                        </td>
                      </tr>
